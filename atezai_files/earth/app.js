...
var InitDemo = function () {
	...
	loadTextResource('./atezai_files/earth/shader.vs.glsl', function (vsErr, vsText) {
		if (vsErr) {
			alert('Fatal error getting vertex shader (see console)');
			console.error(vsErr);
		} else {
			loadTextResource('./atezai_files/earth/shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					alert('Fatal error getting fragment shader (see console)');
					console.error(fsErr);
				} else {
					loadJSONResource('./atezai_files/earth/worldMesh.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting World model (see console)');
							console.error(fsErr);
						} else {
							loadImage('./atezai_files/earth/worldPaintAsmall.jpg', function (imgErr, img) {
								if (imgErr) {
...
var RunDemo = function (vertexShaderText, fragmentShaderText, meshImage, meshModel) {
	...
	var loop = function () {
		...
		angle = performance.now() / 1000 / 30 * 2 * Math.PI;
